            ...
            <div class="page-content">
                ...
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                               <form enctype="multipart/form-data" style="margin-left: 20px;"> 
    								<h5>头像上传：</h5><br>
            						<input id="file-1" class="file" type="file" multiple  
                						data-min-file-count="1" style="max-width:300px;margin-top:-25px;display:none"> <br> 
            						<h5>学号：</h5><input type="text" name="student_id">
           					 		<h5>姓名：</h5><input type="text" name="name">
            						<h5>性别：</h5>
						            <select id="sex">
						            	<option value="男">男</option>
						            	<option value="女">女</option>
						            </select>
						            <h5>生日：</h5><input type="text" name="birthday" value="1999-01-01" data-date-format="yyyy-mm-dd" id="datetimepicker" ><br>
						            <h5>年级：</h5>
						            <select id="grade">
						            	
						            </select><br><br>
						          	<button type='button' class='btn btn-outline btn-info'  onClick='sub()'>提交</button>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body" >
            	<input type="text" name="res" style="border:none;">
            </div>
...
$(document).ready(function(){
	$.getJSON(path+"/findAllGrade.do",null,function(data){
		$("#grade").empty();
		$("#grade").append("<option value='0'>点击选择年级</option>");
		$(data).each(function(index,content){
			var op="<option value='"+content.id+"'>"+content.gradename+"</option>";
			$("#grade").append(op);
		});
	});
});
...
$("#file-1").fileinput({  
    language: 'zh',  
    uploadUrl: 'imgUpload.do', // you must set a valid URL here else you will get an error  
      ...
      function sub(){
    	 ...
    	 var birthday=$("input[name='birthday']").val();
    	  ...
    	  if(studentid==''||name==''||birthday==''||grade==0){
    		  $("input[name='res']").val("请确认信息是否填写完整！");
    		  $("#resModal").modal('show');
    	  }
    	  else{
    		  $.getJSON(path+"/student/addStudent.do",{img:filename,studentid:studentid,name:name,sex:sex,birthday:birthday,grade:grade},function(data){
        		  $("input[name='res']").val("提交成功！");
        		  $("#resModal").modal('show');
        	  });
    	  }
